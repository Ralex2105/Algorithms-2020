...
abstract class AbstractOpenAddressingSetTest {
    ...
    protected fun doRemoveTest() {
        ...
        for (iteration in 1..200) {
            val bitsNumber = nRandom.nextInt(3) + 7
            val openAddressingSet = create<Int>(bitsNumber)
            for (i in 1..70) {
                val firstInt = nRandom.nextInt(23)
                val secondInt = firstInt + (3 shl bitsNumber)
                openAddressingSet += secondInt
                openAddressingSet += firstInt
                val expectedSize = openAddressingSet.size - 1
                assertTrue(
    ...
    protected fun doIteratorTest() {
        ...
        for (iteration in 1..100) {
            ...
            for (i in 1..15) {
                val string = random.nextString("drjhhdrh3453453", 1, 15)
        ...
        for (iteration in 1..300) {
            val controlSet = mutableSetOf<String>()
            for (i in 1..17) {
                val string = nRandom.nextString("aartertabf132435243", 1, 17)
                controlSet.add(string)
            }
            println("Control set: $controlSet")
            val openAddressingSet = create<String>(nRandom.nextInt(1) + 5)
            assertFalse(
                openAddressingSet.iterator().hasNext(),
                "Iterator of an empty set should not have any next elements."
            )
    ...
    protected fun doIteratorRemoveTest() {
        ...
        for (iteration in 1..250) {
            val controlSet = mutableSetOf<String>()
            val removeIndex = nRandom.nextInt(10) + 4
            var toRemove = ""
            for (i in 1..15) {
                val string = nRandom.nextString("argsgbxcvbxbf434564", 1, 15)
                controlSet.add(string)
                if (i == removeIndex) {
                    toRemove = string
                }
            ...
            println("Initial set: $controlSet")
            val openAddressingSet = create<String>(nRandom.nextInt(6) + 4)
            for (element in controlSet) {
                openAddressingSet += element
            }
            controlSet.remove(toRemove)
